...
int main(int argc, char **argv){
    ...
    int start_index, work_rows, nrows, R, k, N = atoi(argv[1]);
    ...
    if (rank == 0) {
        ...
        fprintf(file, "\nMatrice iniziale\n");
        print_graphic_matrix(matrix, N, N, file);
        ...
        MPI_Scatterv(matrix, counts, displacements, MPI_CHAR, recv, nrows*N, MPI_CHAR, 0, MPI_COMM_WORLD);
    } else {
        MPI_Scatterv(NULL, NULL, NULL, NULL, recv, nrows*N, MPI_CHAR, 0, MPI_COMM_WORLD);
    }
    ...
    for(k=0; k<S; k++){
        ...
        MPI_Gatherv(&recv[start_index*N], work_rows*N, MPI_CHAR, matrix, gat_counts, gat_displ, MPI_CHAR, 0, MPI_COMM_WORLD);
        ...
        /*for(int i=start_index; i<start_index + work_rows; i++){
            for(int j=0; j<N; j++){
                printf("[%c]", recv[i*N + j]);
            }
            printf("\n");
