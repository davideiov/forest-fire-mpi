...
int main(int argc, char **argv){
    ...
    int k, empty_counter = 0, empty_matrix = 0, M = atoi(argv[1]), N = atoi(argv[2]);

    file = fopen("correttezza_seq.txt", "w");

    //generazione matrice iniziale
    matrix = (char *) malloc(sizeof(char) * M * N);
    next = (char *) malloc(sizeof(char) * M * N);
        ...
        for(int i=0; i<M; i++){
            for(int j=0; j<N; j++){
                if (matrix[(i*N) + j] == 'E'){
                    ...
                    check_neighbors_ignite(matrix, next, i, j, N, M);
                } else if (matrix[(i*N) + j] == 'B') {
                    next[(i*N) + j] = 'E';
                }
